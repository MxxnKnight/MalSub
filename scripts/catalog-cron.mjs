 ...
 *
 * Modes:
 *   auto        — full scrape when a source has few titles, else incremental
 *   full        — deep scrape (sitemaps + many release pages)
 *   incremental — recent only (RSS + first release pages)
 *
 * Env:
 *   MONGODB_URI   required
 *   MONGO_DB_NAME optional (default MALsub)
 *   CATALOG_MODE  optional (auto|full|incremental)
 *
 * Usage:
 *   node scripts/catalog-cron.mjs
 *   CATALOG_MODE=full node scripts/catalog-cron.mjs
 */
...
const FULL_THRESHOLD = { msone: 500, "movie-mirror": 80, "team-goat": 80 };
...
function parseGenericSitemap(xml, host) {
  ...
  for (const block of xml.split("<url>").slice(1)) {
    ...
    if (path === "/" || /\/(category|tag|page|about|contact|feed|wp-)/i.test(path)) {
      continue;
    }
    const slug = loc.replace(/\/$/, "").split("/").pop() ?? "";
  ...
  if (effective === "full" || byUrl.size < 200) {
    try {
      ...
      const pages = await Promise.all(
        maps.map((url) => fetchTextWithFallback(url, 25000)),
      );
      let mapHits = 0;
  ...
  const sources = [
    {
      id: "msone",
      name: "MSone",
      run: async (effective) => collectMsone(effective),
    },
    {
      id: "movie-mirror",
      name: "Movie Mirror",
