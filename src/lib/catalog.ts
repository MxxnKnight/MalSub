...
async function loadMsoneSeed(): Promise<SearchHit[]> {
  try {
    const mod = (await import("@/lib/data/msone-catalog.json")).default as Array<
      Omit<SearchHit, "poster" | "subtitleUrl"> & {
        poster?: string | null;
        subtitleUrl?: string | null;
      }
  ...
  try {
    const paths = (await import("@/lib/data/msone-paths.json")).default as string[];
    const hits: SearchHit[] = [];
...
async function collectMsone(): Promise<{
  ...
  // 1) RSS — often allowed when sitemaps are blocked
  try {
  ...
  // 2) Releases listing pages (paginated)
  let releaseHits = 0;
  ...
  // 3) Sitemaps (best coverage when Cloudflare allows)
  if (byUrl.size < 200) {
    try {
  ...
  // 4) Bundled seed when live sources are blocked (Render / Cloudflare)
  if (byUrl.size < 500) {
